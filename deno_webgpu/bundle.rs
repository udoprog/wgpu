...
struct WebGpuRenderBundleEncoder {
    raw: RefCell<wgpu_core::command::RenderBundleEncoder>,
...
pub(crate) struct WebGpuRenderBundle {
    ...
    pub(crate) id: wgpu_core::id::RenderBundleId,
...
impl Resource for WebGpuRenderBundle {
    ...
    fn close(self: Rc<Self>) {
        self.core.render_bundle_drop(&self.instance, self.id);
    }
}
...
pub fn op_webgpu_create_render_bundle_encoder(
    ...
    let res = wgpu_core::command::RenderBundleEncoder::new(&descriptor, device.id, None);
    let (render_bundle_encoder, maybe_err) = match res {
        Ok(encoder) => (encoder, None),
        Err(e) => (
            wgpu_core::command::RenderBundleEncoder::dummy(device.id),
            Some(e),
        ),
    };

    let rid = state.resource_table.add(WebGpuRenderBundleEncoder {
        raw: RefCell::new(render_bundle_encoder),
        core: device.core,
    });
...
pub fn op_webgpu_render_bundle_encoder_finish(
    ...
    let render_bundle_encoder = Rc::try_unwrap(render_bundle_encoder)
        .ok()
        .expect("unwrapping render_bundle_encoder_resource should succeed");

    let raw = render_bundle_encoder.raw.into_inner();

    let instance = state.borrow::<super::Instance>();

    gfx_put!(render_bundle_encoder => instance.render_bundle_encoder_finish(
        raw,
...
pub fn op_webgpu_render_bundle_encoder_set_bind_group(
    ...
    unsafe {
        wgpu_core::command::bundle_ffi::wgpu_render_bundle_set_bind_group(
            &mut render_bundle_encoder.raw.borrow_mut(),
            index,
            bind_group.id,
