...
impl<T: Resource> Registry<T> {
    pub(crate) fn prepare<G>(&self, id_in: G::Input) -> FutureId<T>
    where
        G: IdentityHandlerFactory,
    {
        FutureId {
            id: if G::autogenerate_ids() {
                self.identity.process(self.backend)
            } else {
                let id = unsafe { Id::from_raw(G::input_to_id(id_in)) };
                self.identity.mark_as_used(id)
            },
